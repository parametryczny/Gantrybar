...
class PrinterCard(Gtk.Frame):
    def __init__(self, app: "BambuBar", printer: Printer) -> None:
        super().__init__()
        ...
        target = Gtk.TargetEntry.new("application/x-bambubar-printer", Gtk.TargetFlags.SAME_APP, 0)
...
class Dashboard(Gtk.Window):
    def __init__(self, app: "BambuBar") -> None:
...
class PrinterDialog(Gtk.Dialog):
    def __init__(self, app: "BambuBar", printer: Printer | None = None) -> None:
...
class BambuBar:
    ...
    def _tray(self) -> None:
        ...
        menu.show_all()
        if AppIndicator:
            installed_icon = Path("/usr/share/icons/hicolor/scalable/apps/bambubar.svg")
            local_icons = Path(__file__).resolve().parent.parent / "assets"
            icon = str(installed_icon) if installed_icon.exists() else str(local_icons / "bambubar.svg")
            self.indicator = AppIndicator.Indicator.new("bambubar", icon, AppIndicator.IndicatorCategory.APPLICATION_STATUS)
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="bambubar")
    ...
    if args.kiosk:
        from .kiosk import KioskBambuBar
        KioskBambuBar()
    else:
        BambuBar(background=args.background)
    Gtk.main(); return 0
