...
class WebConfigServer:
    ...
    def start(self) -> None:
        try:
            ...
            self.httpd = server
            self.thread = threading.Thread(target=server.serve_forever, name="bambubar-web-config", daemon=True)
            self.thread.start()
...
def ensure_certificate() -> tuple[str, str]:
    ...
    hostname = socket_hostname()
    # Retain bambubar.local as a compatibility SAN for existing bookmarks.
    names = f"DNS:{hostname},DNS:{hostname}.local,DNS:gantry.local,DNS:bambubar.local,DNS:localhost,IP:127.0.0.1"
    subprocess.run([
...
def socket_hostname() -> str:
    import socket
    return socket.gethostname().strip().lower() or "bambubar"
