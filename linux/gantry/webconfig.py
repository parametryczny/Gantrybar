...
def validate_printer_form(values: dict[str, str], language: str = "pl") -> tuple[dict[str, Any] | None, str]:
    ...
    if kind not in {"bambu", "klipper", "prusa"}:
        return None, tr(language, "unsupported_kind")
    ...
    if not name or not host or not serial or (kind in {"bambu", "prusa"} and not code) or not 1 <= port <= 65535:
        return None, tr(language, "form_required")
    if any(character.isspace() for character in host) or len(serial) > 128:
        return None, tr(language, "form_format")
...
class WebConfigServer:
    def __init__(self, snapshot: Snapshot, add_printer: AddPrinter, remove_printer: RemovePrinter,
                 ...
                 language_provider: Callable[[], str] | None = None) -> None:
        ...
        self.host = host
        self.port = port
        self.language_provider = language_provider or (lambda: "pl")
    ...
    def current_language(self) -> str:
        return normalize_language(self.language_provider())
...
class ConfigHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        language = self.server_owner.current_language()
        if self.path == "/favicon.ico":
            self.send_response(204); self.end_headers(); return
        token = self._session_token()
        if not self.server_owner.valid_session(token):
            self._html(login_page(language=language), status=200); return
        if self.path == "/template.csv":
            ...
            return
        self._html(config_page(self.server_owner.snapshot(), token or "", language=language))
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/login":
            if not self.server_owner.login_allowed(address):
                self._html(login_page(tr(language, "too_many_attempts"), language), status=429); return
            ...
            if not secrets.compare_digest(supplied, self.server_owner.code):
                ...
                self._html(login_page(tr(language, "invalid_pairing"), language), status=403); return
        ...
        if not self.server_owner.valid_session(token) or not secrets.compare_digest(form.get("csrf", ""), token or ""):
            self._html(login_page(tr(language, "session_expired"), language), status=403); return
        if self.path == "/add":
            values, error = validate_printer_form(form, language)
            ok, message = self.server_owner.add_printer(values) if values else (False, error)
            self._html(config_page(self.server_owner.snapshot(), token or "", message, ok, language), status=200 if ok else 400); return
        if self.path == "/remove":
            ok, message = self.server_owner.remove_printer(form.get("serial", ""))
            self._html(config_page(self.server_owner.snapshot(), token or "", message, ok, language), status=200 if ok else 400); return
        if self.path == "/import":
            try:
                records = parse_printer_csv(form.get("csv", ""), language=language)
                ok, message = self.server_owner.import_printers(records)
            except ValueError as error:
                ok, message = False, str(error)
            self._html(config_page(self.server_owner.snapshot(), token or "", message, ok, language), status=200 if ok else 400); return
        self._html(tr(language, "not_found"), status=404)
...
def page_shell(content: str, title: str | None = None, language: str = "pl") -> str:
    language = normalize_language(language)
    title = title or tr(language, "web_title")
    ...
    return f"""<!doctype html><html lang=\"{language}\"><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">{refresh}<title>{html.escape(title)}</title><style>
...
def login_page(message: str = "", language: str = "pl") -> str:
    notice = f'<div class="message">{html.escape(message)}</div>' if message else ""
    return page_shell(f"""<header><div><h1>GANTRY</h1><small>{html.escape(tr(language, "rpi_configuration"))}</small></div></header>{notice}<section class=\"panel\"><form method=\"post\" action=\"/login\"><label>{html.escape(tr(language, "pairing_label"))}<input name=\"code\" inputmode=\"numeric\" autocomplete=\"one-time-code\" maxlength=\"7\" required autofocus></label><p><button type=\"submit\">{html.escape(tr(language, "connect"))}</button></p></form><small>{html.escape(tr(language, "certificate_hint"))}</small></section>""", language=language)


def config_page(snapshot: dict[str, Any], csrf: str, message: str = "", success: bool = False,
                language: str = "pl") -> str:
    ...
    def row(item: dict[str, Any]) -> str:
        ...
        state_label = tr(language, "ready" if state == "idle" else state) if state in {"idle", "printing", "paused", "finished", "error", "offline"} else state
        return f'''<article class="printer-card {html.escape(state)}"><div class="printer-head"><span><strong>{html.escape(str(item.get("name", "")))}</strong><small>{html.escape(str(item.get("kind", "bambu"))).title()} • {html.escape(state_label)}</small></span><b>{progress}%</b></div><div class="job">{html.escape(str(item.get("job", "—")))}</div><progress max="100" value="{progress}"></progress><div class="telemetry">◷ {eta}　▤ {layer}　♨ {nozzle}　▣ {bed}</div><div class="slots">{slots}</div><form method="post" action="/remove"><input type="hidden" name="csrf" value="{html.escape(csrf)}"><input type="hidden" name="serial" value="{html.escape(str(item.get("serial", "")))}"><button class="remove" type="submit">{html.escape(tr(language, "remove"))}</button></form></article>'''

    rows = "".join(row(item) for item in printers if isinstance(item, dict)) or f'<p class="muted">{html.escape(tr(language, "no_printers_yet"))}</p>'
    notice = f'<div class="message{" ok" if success else ""}">{html.escape(message)}</div>' if message else ""
    return page_shell(f"""<header><div><h1>GANTRY</h1><small>{html.escape(tr(language, "local_panel", count=len(printers)))}</small></div><code>RPi</code></header>{notice}<section class=\"panel\"><h2>{html.escape(tr(language, "printer_status"))}</h2><div class=\"printer-grid\">{rows}</div></section><section class=\"panel\"><h2>{html.escape(tr(language, "import_csv"))}</h2><p class=\"muted\">{html.escape(tr(language, "csv_description"))}</p><p><a class=\"button secondary\" href=\"/template.csv\">{html.escape(tr(language, "download_csv_template"))}</a></p><form id=\"csv-form\" method=\"post\" action=\"/import\"><input type=\"hidden\" name=\"csrf\" value=\"{html.escape(csrf)}\"><input id=\"csv-file\" type=\"file\" accept=\".csv,text/csv\" required><textarea id=\"csv-content\" name=\"csv\" hidden></textarea><p><button type=\"submit\">{html.escape(tr(language, "import_printers"))}</button></p></form></section><section class=\"panel\"><h2>{html.escape(tr(language, "add_manually"))}</h2><form method=\"post\" action=\"/add\" class=\"grid\"><input type=\"hidden\" name=\"csrf\" value=\"{html.escape(csrf)}\"><label>{html.escape(tr(language, "type"))}<select name=\"kind\"><option value=\"bambu\">Bambu Lab</option><option value=\"klipper\">Klipper / Moonraker</option><option value=\"prusa\">Prusa / PrusaLink</option></select></label><label>{html.escape(tr(language, "name"))}<input name=\"name\" required></label><label>{html.escape(tr(language, "host"))}<input name=\"host\" required></label><label>{html.escape(tr(language, "serial"))} (Bambu)<input name=\"serial\"></label><label>{html.escape(tr(language, "port"))}<input name=\"port\" type=\"number\" min=\"1\" max=\"65535\" placeholder=\"8883 / 7125 / 80\"></label><label class=\"wide\">{html.escape(tr(language, "access_api"))}<input name=\"code\" type=\"password\"></label><div class=\"wide\"><button type=\"submit\">{html.escape(tr(language, "add_connect"))}</button></div></form></section><script>const form=document.getElementById('csv-form'),file=document.getElementById('csv-file'),content=document.getElementById('csv-content');form.addEventListener('submit',async event=>{{if(!file.files.length){{event.preventDefault();return}}event.preventDefault();content.value=await file.files[0].text();form.submit()}});</script>""", language=language)
