...
- **kolejka druku w oknie Farma**: płytę z pociętego 3MF dodajesz do kolejki w wybranej liczbie kopii, dla dowolnej drukarki Bambu Lab albo tylko dla zaznaczonych. Przy każdej drukarce jest przełącznik „Stół pusty — kolejka może startować”. Gdy go zaznaczysz, a drukarka jest wolna, Gantry wybiera pierwszą pasującą pozycję: ten sam materiał w AMS w podobnym kolorze (i dość filamentu, gdy rolka podaje wagę), ta sama średnica dyszy; płyta z jednym filamentem może iść ze szpuli zewnętrznej. Plik jest wysyłany, a po dotarciu druk startuje sam, z tymi samymi sprawdzeniami co start ręczny. Jedno zaznaczenie pozwala na jeden start — po zdjęciu wydruku trzeba znowu potwierdzić pusty stół. Znacznik stołu nie przetrwa restartu aplikacji, a nieudany transfer oddaje kopię do kolejki.
- **cena rolki w Spoolbase**: w oknie „Dane filamentu” są pola „Cena rolki” i „Kod EAN” (osobno od kodu producenta, ze sprawdzeniem cyfry kontrolnej); cena produktu trafia do każdej nowej rolki i liczy się w koszcie wydruku, gdy rolka nie ma własnej. Cenę można też podać przy dodawaniu filamentu z katalogu (pole „Cena rolki” obok wagi), przy tworzeniu nowej rolki dla slotu (podpowiada cenę ostatniej rolki tego produktu) i na ekranie „Popraw wagę” dla istniejącej rolki. Cena widnieje przy zapasowych rolkach, a koszt wydruku liczy filament z ceny faktycznie użytej rolki (cena ÷ waga pełnej rolki); rolka bez ceny liczy się jak dotąd, z cennika materiałów. Windows zachowuje cenę przy zapisie magazynu.
- **kod EAN z ręki**: w katalogu filamentów obok „Skanuj kod…” jest „Wpisz kod…” — dla etykiety, która nie chce się zeskanować, komputera bez kamery albo czytnika kodów na USB. Kod jest sprawdzany jak zeskanowany: znaleziony zaznacza filament w katalogu, nieznaleziony proponuje dodanie własnego z tym kodem. Przy EAN-8/UPC-A/EAN-13 Gantry sprawdza cyfrę kontrolną i ostrzega przed literówką.
- **koszt wydruku**: w Statystykach floty przycisk „Ceny…” (waluta, filament za kg z cenami per materiał, prąd za kWh, średnia moc drukarki, czas maszyny za godzinę). Każdy wydruk dostaje koszt: filament z rozliczeń Spoolbase, prąd i czas maszyny z czasu druku. Podsumowanie pokazuje koszt razem i w podziale, średni koszt udanego wydruku oraz ile poszło na nieudane; wydruk bez przypisanej rolki liczy się bez filamentu i jest o tym informacja. Eksport „CSV…” zapisuje wydruk po wydruku z kosztami (średnik i przecinek dziesiętny po polsku, więc Excel od razu otwiera kolumny).
- **statystyki produkcji**: wykorzystanie drukarek (ile czasu okresu drukowały), wydruki w tygodniu za ostatnie 8 tygodni, zużycie filamentu wg materiału, najczęściej drukowane pliki, podział nieudanych na błędy i przerwane oraz lista ostatnich wydruków z czasem, gramami i kosztem. Filament liczy się teraz także dla wybranego okresu, nie tylko „Wszystko”. Historia wydruków na drukarkę mieści 1000 pozycji zamiast 100.
