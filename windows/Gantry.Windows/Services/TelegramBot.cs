...
public sealed class TelegramBot
{
    ...
    private async Task SendPrinterMenuAsync(int? messageId)
    {
        ...
        if (printers.Count == 0) { await SendAsync(AppSettings.T("No printers."), CommandKeyboard()); return; }
        var rows = printers.Select(p => new[] { Btn($"{Icon(p.Serial)} {p.Name}", $"p:{p.Serial}") }).ToArray();
        var text = AppSettings.T("Pick a printer:");
            ...
            case "stopask":
                if (messageId is { } mid)
                    await EditAsync(mid, string.Format(AppSettings.T("⏹ Cancel the print on {0}? This cannot be undone."), name),
                        Keyboard(new[] { new[] {
                            Btn(AppSettings.T("Yes, cancel"), $"a:stop:{serial}"),
    ...
    private async Task HandlePhotoAsync(string serial, string cbId)
    {
        ...
        await AnswerAsync(cbId, "📷…");
        await SendAsync(string.Format(AppSettings.T("📷 Grabbing a camera snapshot from {0}…"), name), CommandKeyboard());
    ...
    private async Task SendAllAsync()
    {
        ...
        if (lines == null) { await SendAsync(AppSettings.T("No printers."), CommandKeyboard()); return; }
        await SendAsync(AppSettings.T("🖨 Fleet:") + "\n" + string.Join("\n", lines), CommandKeyboard());
    }
    ...
    private async Task SendSpoolsAsync()
    {
        ...
        if (lines.Count == 0) { await SendAsync(AppSettings.T("✅ No spools running low (≤20%)."), CommandKeyboard()); return; }
        await SendAsync(AppSettings.T("🧵 Spools running low:") + "\n" + string.Join("\n", lines), CommandKeyboard());
    }

    private async Task SendHistoryAsync()
    {
        var entries = PrintHistory.Recent(10);
        if (entries.Count == 0) { await SendAsync(AppSettings.T("No print history yet."), CommandKeyboard()); return; }
        var lines = entries.Select(e => $"{e.Date:dd.MM HH:mm} · {e.Printer}" + (string.IsNullOrEmpty(e.Job) ? "" : $" · {e.Job}"));
        await SendAsync(AppSettings.T("📜 Recent prints:") + "\n" + string.Join("\n", lines), CommandKeyboard());
    }
    ...
    private async Task HandleMuteAsync(string? argument)
    {
        if (string.Equals(argument, "off", StringComparison.OrdinalIgnoreCase))
        {
            ...
            await SendAsync(AppSettings.T("🔔 Mute off."), CommandKeyboard());
            return;
        }
        ...
        if (seconds == null)
        {
            if (AppSettings.TelegramMuteUntil is { } until)
                await SendAsync(string.Format(AppSettings.T("🔕 Muted until {0:HH:mm}. Turn off: /mute off"), until.ToLocalTime()), CommandKeyboard());
            else
                await SendAsync(AppSettings.T("Give a duration, e.g. /mute 2h or /mute 30m. Turn off: /mute off"), CommandKeyboard());
            return;
        }
        var muteUntil = DateTime.UtcNow.AddSeconds(seconds.Value);
        AppSettings.TelegramMuteUntil = muteUntil;
        await SendAsync(string.Format(AppSettings.T("🔕 Alerts muted until {0:HH:mm}."), muteUntil.ToLocalTime()), CommandKeyboard());
    }
    ...
    private async Task HandleWatchAsync(string? argument)
    {
        ...
        {
            _watchCts?.Cancel(); _watchCts = null;
            await SendAsync(AppSettings.T("📷 Watch off."), CommandKeyboard());
            return;
        }
        var seconds = ParseDuration(argument);
        if (seconds == null || seconds < 60)
        {
            await SendAsync(AppSettings.T("Give an interval ≥ 1 min, e.g. /watch 10m. Turn off: /watch off"), CommandKeyboard());
            return;
        }
        _watchCts?.Cancel();
        _watchCts = new CancellationTokenSource();
        _ = WatchLoopAsync(seconds.Value, _watchCts.Token);
        await SendAsync(string.Format(AppSettings.T("📷 Watch: photos of printing machines every {0}. Turn off: /watch off"), argument), CommandKeyboard());
    }
    ...
    private string StatusText(string serial)
    {
        ...
        if (t.State is PrinterState.Printing or PrinterState.Paused)
        {
            var line = $"{AppSettings.T("Progress")}: {t.Progress}%";
            if (t.CurrentLayer != null) line += $" · {AppSettings.T("layer")} {t.CurrentLayer}/{t.TotalLayers ?? 0}";
            lines.Add(line);
            if (t.RemainingMinutes is { } m && m > 0) lines.Add($"ETA: {m / 60}h {m % 60}m");
            if (!string.IsNullOrEmpty(t.JobName)) lines.Add(t.JobName!);
        }
        lines.Add($"{AppSettings.T("Nozzle")} {Temp(t.NozzleTemperature, t.NozzleTargetTemperature)} · {AppSettings.T("bed")} {Temp(t.BedTemperature, t.BedTargetTemperature)}"
            + (t.ChamberTemperature != null ? $" · {AppSettings.T("chamber")} {Temp(t.ChamberTemperature, null)}" : ""));
    ...
    {
        PrinterState.Printing => AppSettings.T("Printing"),
        PrinterState.Paused => AppSettings.T("Paused"),
        PrinterState.Finished => AppSettings.T("Finished"),
        PrinterState.Error => AppSettings.T("Error"),
        PrinterState.Idle => AppSettings.T("Ready"),
        _ => "Offline"
    };
